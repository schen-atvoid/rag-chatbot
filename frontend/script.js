...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (sources && sources.length > 0) {
        const sourceLinks = sources.map(s =>
            s.url
                ? `<a href="${s.url}" target="_blank" rel="noopener noreferrer" class="source-link">${escapeHtml(s.text)}</a>`
                : `<span class="source-link source-link--missing">${escapeHtml(s.text)}</span>`
        ).join('<br>');
        html += `
